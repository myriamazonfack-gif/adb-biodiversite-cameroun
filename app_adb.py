...
def charger_donnees(fichier_bytes):
    ...
    # On recrée un BytesIO à chaque lecture de feuille (le curseur doit être au début)
    def excel_bytes():
    ...
    for nom in ["PowerBI_Data", "D. Publiques"]:
        if nom in feuilles:
            if nom == "PowerBI_Data":
                df = pd.read_excel(excel_bytes(), sheet_name=nom, header=1)
    ...
    if df is None:
        ...
        for feuille, secteur in mapping.items():
            if feuille in feuilles:
                tmp = pd.read_excel(excel_bytes(), sheet_name=feuille, header=2)
    ...
    for nom in ["Donnees Macro", "Données Macro"]:
        if nom in feuilles:
            macro = pd.read_excel(excel_bytes(), sheet_name=nom, header=1)
...
if fichier_charge is None:
    # Mode démonstration — chercher le fichier Excel automatiquement
    # On essaie plusieurs emplacements possibles
    _script_dir = os.path.dirname(os.path.abspath(__file__)) if "__file__" in dir() else os.getcwd()
    _candidates = [
        os.path.join(_script_dir, "ADB_Cameroun_Simulation.xlsx"),
    ...
    if demo_bytes is not None:
        ...
        # Aucun fichier trouvé → page d'accueil propre
        st.markdown("""
        ...
        """, unsafe_allow_html=True)
        st.info("📁 Placez le fichier **ADB_Cameroun_Simulation.xlsx** dans le même dossier que `app_adb.py`, ou chargez-le via le panneau latéral.")
        st.stop()
else:
