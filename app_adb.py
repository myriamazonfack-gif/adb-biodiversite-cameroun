...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown('<div class="filtre-bar"><div class="filtre-title">🎛️ Filtres — Sélectionnez vos critères</div>', unsafe_allow_html=True)

f1, f2, f3, f4, f5 = st.columns([2, 2, 3, 2, 2])
...
with f4:
    insts_sel = st.multiselect("🏢 Institutions", insts_dispo, default=insts_dispo) if insts_dispo else []
with f5:
    unite = st.radio("💰 Unité", ["Millions FCFA", "Milliards FCFA"], index=0)
...
with tab1:
    ...
    st.dataframe(
        pivot_resume.style.format("{:,.1f}").background_gradient(
            cmap="Greens", subset=[c for c in pivot_resume.columns if c != "TOTAL"]
